...
{
    public class PostgreSQLDbHeplerTest
    {
        public PostgreSQLDbHeplerTest()
        {
            var connectionString = "Host=localhost;Username=postgres;Password=123456;Database=DapperTemp";

            DbContext = new DbContext(new DbContextOptions()
            {
                ConnectionString = connectionString,
                DbFactory = new PostgreSQLDbFactory(connectionString)
            });
            DbHelper = new PostgreSQLDbHelper<DbContext>(DbContext);

            DbHelper.Execute("truncate table \"User\"; truncate table \"Org\";");
            DbHelper.Execute("alter sequence \"org_Id_seq\" restart with 1");
            DbHelper.Execute("alter sequence \"orguser_Id_seq\" restart with 1");
            DbHelper.Execute("alter sequence \"user_Id_seq\" restart with 1");
            DbHelper.Execute("alter sequence \"user2_Id_seq\" restart with 1");
        }

        public DbContext DbContext { get; }

        public PostgreSQLDbHelper<DbContext> DbHelper { get; }
        ...
        public void ExecuteSpTest()
        {
            ...
            DbHelper.Execute(sp, new { Name = "ExecuteSpOrg" }, null, System.Data.CommandType.StoredProcedure);
        }
        ...
        {
            var sp = "\"ExecuteSqlTestSP\"";
            await DbHelper.ExecuteAsync(sp, new { Name = "ExecuteAsyncSpOrg" }, null, System.Data.CommandType.StoredProcedure);
        }

        [Fact]
        public void ExecuteFuncTest()
        {
            var func = "select \"ExecuteSqlTestFunc\"(@Name)";
            var r = DbHelper.ExecuteScalar<long>(func, new { Name = "ExecuteSpOrg" });
            Assert.Equal(1, r);
        }

        [Fact]
        public async void ExecuteAsyncFuncTest()
        {
            var func = "select \"ExecuteSqlTestFunc\"(@Name)";
            var r = await DbHelper.ExecuteScalarAsync<long>(func, new { Name = "ExecuteSpOrg" });
            Assert.Equal(1, r);
        }
        ...
        {
            var sql = @"
                insert into ""Org""(""Name"") values(@Name) RETURNING CAST(""Id"" AS BIGINT);
            ";
